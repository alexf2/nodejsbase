...
import {Logger, UnauthorizedLoginError, UnauthorizedPwdError, getRequestIdMeta, safeAsyncHandler} from '../helpers';
...
export class AuthenticationController extends ControllerBase {
    ...
    public readonly login = safeAsyncHandler(async (req: ValidatedRequest<LoginBodySchema>, res: Response, next: NextFunction) => {
        ...
        if (user) {
            if (compareSync(password, user.password)) {
                res.status(201).json(btoken.sign(this.createAccessToken(user), TOKEN_KEY));
            } else {
                throw new UnauthorizedPwdError(login);
            }
        } else {
            throw new UnauthorizedLoginError(login);
        }
    })
